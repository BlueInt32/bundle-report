...
s.linkInternalFilters = "javascript:,mycanal.fr"
...
{
	s.detectRIA('s_ria', 'prop8', 'prop20', '', '', ''); var urlPage = window.location; var tempVar = s.getQueryParam('origref', '', urlPage); if (tempVar) { s.referrer = tempVar }; s.server = window.location.hostname; if (s.channel) { s.eVar3 = s.channel; s.hier1 = "Web Canal|" + s.channel; if (s.prop4) { s.hier1 = s.hier1 + '|' + s.prop4; if (s.prop5) { s.hier1 = s.hier1 + '|' + s.prop5; if (s.prop6) { s.hier1 = s.hier1 + '|' + s.prop6; if (s.prop7) { s.hier1 = s.hier1 + '|' + s.prop7; } } } } }
	s.prop23 = s.pageName; var sc_cmpid = s.getQueryParam('sc_cmpid', '', urlPage); var clkid = s.getQueryParam('clkid', '', urlPage); var cpmid = s.getQueryParam('cpmid', '', urlPage); var kwid = s.getQueryParam('kwid', '', urlPage); var sc_intcmp = s.getQueryParam('sc_intcmp', '', urlPage); var urlHash = window.location.hash; if (urlHash && urlHash.indexOf('=') > 0)
	{
		urlHash = '?' + urlHash.substr(1); if (sc_cmpid.length == 0)
			sc_cmpid = s.getQueryParam('sc_cmpid', '', urlHash); if (clkid.length == 0)
				clkid = s.getQueryParam('clkid', '', urlHash); if (cpmid.length == 0)
					cpmid = s.getQueryParam('cpmid', '', urlHash); if (kwid.length == 0)
						kwid = s.getQueryParam('kwid', '', urlHash); if (sc_intcmp.length == 0)
							sc_intcmp = s.getQueryParam('sc_intcmp', '', urlHash);
	}
	if (sc_cmpid) { s.campaign = sc_cmpid; s.prop23 = "CMP: " + s.pageName; }
	...
	s.campaign = s.getValOnce(s.campaign, 's_campaign', 0); if (!s.eVar2) { s.eVar2 = s.getValOnce(sc_intcmp, 'ev2', 0); }
	if (!s.eVar41) { s.eVar41 = s.getValOnce(s.eVar41, 'ev41', 0); }
	var esvcid = s.getQueryParam('esvcid', '', urlPage); if (esvcid) { s.eVar18 = esvcid }; if (!s.prop1) { s.prop1 = s.getQueryParam('mot', '', urlPage); }
	...
	if (s.prop1) { s.prop1 = s.prop1.toLowerCase(); var t_search = s.getValOnce(s.prop1, 'ev1', 0); if (t_search) { s.events = s.apl(s.events, 'event1', ',', 1); } }
	if (s.pageName) { s.eVar8 = s.pageName; s.events = s.apl(s.events, "event3", ",", 1); }
	s.prop22 = s.getQueryParam('zonepp'); s.prop30 = s.getNewRepeat(); s.eVar30 = s.prop30; if (s.prop1) { s.eVar1 = "D=c1"; }
	...
	s.eVar33 = s.crossVisitParticipation(s.campaign, 's_cpm', '90', '10', '>', 'purchase'); s.eVar34 = s.referrer; tempVar = s.getQueryParam('ESVreferrer', '', urlPage); if (tempVar) { s.eVar34 = tempVar; }
}
...
+ "oin(h,{delim:dl});return r;"); s.getQueryParam = new Function("p", "d", "u", ""
...
+ "==0?0:a);}return v==k?'':v"); s.p_fo = new Function("n", ""
+ "var s=this;if(!s.__fo){s.__fo=new Object;}if(!s.__fo[n]){s.__fo[n]="
+ "new Object;return 1;}else {return 0;}"); s.getNewRepeat = new Function("d", "cn", ""
...
+ "++]=l.substring(0,i);l=l.substring(i+d.length);}return a"); s.detectRIA = new Function("cn", "fp", "sp", "mfv", "msv", "sf", ""
...
+ "var s=this;if(!s.__fo){s.__fo=new Object;}if(!s.__fo[n]){s.__fo[n]="
+ "new Object;return 1;}else {return 0;}"); var s_code = '', s_objectID; function s_gi(un, pg, ss)
{
